...
ggsave("project/output/eda_age.png", width = 16, height = 8, units = "cm", bg = "white")
...
ggsave("project/output/eda_eclair.png", width = 16, height = 8, units = "cm", bg = "white")
...
ggsave("project/output/eda_sex.png", plot = sex_bar, width = 16, height = 8, units = "cm", bg = "white")
ggsave("project/output/eda_curry.png", plot = curry_bar, width = 16, height = 8, units = "cm", bg = "white")
ggsave("project/output/eda_water.png", plot = water_bar, width = 16, height = 8, units = "cm", bg = "white")
...
beta.post <- function(beta.input, X.data, y.data){
  ...
  # y.data is the (0/1) vector indicating if the outcome of interest occurred (food poisoning)
  
  # check that vector of parameters has length equal to number of columns in design matrix 
  if(length(beta.input) != ncol(X.data)){
    stop("The number of parameters must match the number of columns in the design matrix")
  }
...
beta.sampler <- function(X.data, y.data, beta.phat, sigma2 = 10000, n.sims = 100000){
  # beta.phat is the vector containing the parameter estimates from logistic regression
  ...
  beta.draws <- matrix(NA, nrow = n.sims, ncol = length(beta.phat))
  ...
  for (i in 2:n.sims) {
    ...
    beta.star <-  mvrnorm(n = 1, mu = beta.cur, Sigma = sigma2*diag(length(beta.cur)))
    
    # calculate acceptance ratio 
    log.R <- beta.post(beta.star, X.data, y.data) - beta.post(beta.cur, X.data, y.data)
...
# estimate for each of our parameters, rather than setting non-significant parameters
# in logistic reg. to zero 
...
data <- data %>%
  ...
  mutate(intercept = 1) %>%
  # arrange to be same as beta.phat vector
  dplyr::select(case, intercept, age, beefcurry, eclair, sex, water) %>%
...
# get vector of response 
my.y <- data$case

# MH draws 
beta.draws <- beta.sampler(X.data = my.X, y.data = my.y, my.beta.phat)
beta.draws

### different values of sigma^2 ----
post.beta <- mcmc(beta.draws) # convert to mcmc object
plot(post.beta)
