...
for (i in 1:length(diff.sigma2)) {
  ...
  # get draws 
  beta.draws <- beta.sampler(X.data = my.X, y.data = my.y, my.beta.phat, sigma2 = sigma2.oi)$beta_draws
  
  # convert to mcmc object 
  post.beta <- mcmc(beta.draws)
  
  # save first plot
  plot(post.beta, col = "#F588AFFF") # create the first plot
  png("first_plot.png", width = 16, height = 8, units = "cm", res = 300) # open device for the first plot
  
  # Second plot
  plot(post.beta, col = "#F588AFFF") # Create the first plot
  png("second_plot.png", width = 16, height = 8, units = "cm", res = 300) 
