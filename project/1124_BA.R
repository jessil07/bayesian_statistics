...
# estimate for each of our parameters, rather than setting non-significant 
# parameters in logistic reg. to zero 
m1 <- glm(case~ age + beefcurry + eclair +sex + water, data=data, family= binomial(link="logit"))
...
# MH draws, choose sigma2 = 0.1 as example  
beta.sampler.output <- beta.sampler(X.data = my.X, y.data = my.y, 
                                    my.beta.phat, sigma2 = 0.1)
beta.draws <- beta.sampler.output$beta_draws

post.beta <- mcmc(beta.draws, start = 1000, end = 100000)
plot(post.beta, col = "#F588AFFF") # create the first plot


### different values of sigma^2 ----
# different sigma^2
diff.sigma2 <- c(0.1) # need to check this?

# loop for getting plots 
for (i in 1:length(diff.sigma2)) {
  ...
  # name plots 
  first_name <- paste0("project/output/conv/", sigma2.oi.cleaned, "_first_tp.png")
  second_name <- paste0("project/output/conv/", sigma2.oi.cleaned, "_second_tp.png")
  
  # save first plot
  par(mar = c(4, 4, 2, 1))  # reduce margins
  plot(post.beta, col = "#F588AFFF") # create the first plot
  png(first_name, width = 16, height = 8, units = "cm", res = 300) # open device for the first plot
  
  # second plot
  par(mar = c(4, 4, 2, 1))  # reduce margins
  plot(post.beta, col = "#F588AFFF") # create the first plot
  png(second_name, width = 16, height = 8, units = "cm", res = 300) 
  plot(post.beta, col = "#F588AFFF") # create the second plot
  dev.off() # close the device
  
  # acf plot 
  par(mar = c(4, 4, 2, 1))  # reduce margins
  png(paste0("project/output/conv/", sigma2.oi.cleaned, "_acf.png"), width = 16, height = 8, units = "cm", res = 300)
  autocorr.plot(post.beta)
  dev.off()
  
  cat("variance = ", sigma2.oi, 
      "ess = ", effectiveSize(post.beta), 
      "no of accepted samples = ", beta.sampler.output$acceptance.count)
}
